;; Original file: phi_78.smt2
(set-logic HORN)
(declare-fun h1 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h2 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h3 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h4 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h5 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h6 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h7 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h8 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h9 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h10 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h11 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h12 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h13 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h14 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h15 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h16 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h17 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h18 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h19 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h20 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h21 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h22 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h23 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)
(declare-fun h24 (Int Int Int Int Int Int Int Int Int Int Int Int ) Bool)


(assert (forall ((A Int) (B Int) (C Int) (D Int) (E Int) (F Int))
  (=> (= 1 1) (h1 A B C D E F A B C D E F))))
(assert (forall ((H Int)
         (I Int)
         (J Int)
         (K Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (M Int)
         (N Int)
         (O Int)
         (P Int)
         (L Int))
  (=> (and (= H 0) (= I 0) (= J 0) (= K 0) (h1 A B C D E F G M N O P L))
      (h2 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h2 A B C D E F G H I J K L) (h3 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h3 A B C D E F G H I J K L) (h4 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h4 A B C D E F G H I J K L) (h5 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h5 A B C D E F G H I J K L) (h6 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h6 A B C D E F G H I J K L) (h7 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h7 A B C D E F G H I J K L) (h8 A B C D E F G H I J K L))))
(assert (forall ((L Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int))
  (=> (and (<= L (- 1)) (h8 A B C D E F G H I J K L))
      (h9 A B C D E F G H I J K L))))
(assert (forall ((M Int)
         (H Int)
         (N Int)
         (K Int)
         (O Int)
         (I Int)
         (P Int)
         (J Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (L Int))
  (let ((a!1 (and (= M (+ (- 1) H))
                  (= N (+ (- H) K))
                  (= O (+ (- 1) I))
                  (= P (+ (- I) J))
                  (h9 A B C D E F G M O P N L))))
    (=> a!1 (h10 A B C D E F G H I J K L)))))
(assert (forall ((L Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int))
  (=> (and (>= L 1) (h8 A B C D E F G H I J K L)) (h11 A B C D E F G H I J K L))))
(assert (forall ((M Int)
         (H Int)
         (N Int)
         (K Int)
         (O Int)
         (I Int)
         (P Int)
         (J Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (L Int))
  (let ((a!1 (and (= M (+ (- 1) H))
                  (= N (+ (- H) K))
                  (= O (+ (- 1) I))
                  (= P (+ (- I) J))
                  (h11 A B C D E F G M O P N L))))
    (=> a!1 (h10 A B C D E F G H I J K L)))))
(assert (forall ((L Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int))
  (=> (and (= L 0) (h8 A B C D E F G H I J K L)) (h12 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h12 A B C D E F G H I J K L) (h13 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h13 A B C D E F G H I J K L) (h14 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h10 A B C D E F G H I J K L) (h15 A B C D E F G H I J K L))))
(assert (forall ((G Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (and (<= G (- 1)) (h15 A B C D E F G H I J K L))
      (h16 A B C D E F G H I J K L))))
(assert (forall ((M Int)
         (J Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (K Int)
         (L Int))
  (let ((a!1 (and (= M (+ (- 1) J)) (h16 A B C D E F G H I M K L))))
    (=> a!1 (h17 A B C D E F G H I J K L)))))
(assert (forall ((G Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (and (>= G 1) (h15 A B C D E F G H I J K L))
      (h18 A B C D E F G H I J K L))))
(assert (forall ((M Int)
         (J Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (K Int)
         (L Int))
  (let ((a!1 (and (= M (+ (- 1) J)) (h18 A B C D E F G H I M K L))))
    (=> a!1 (h17 A B C D E F G H I J K L)))))
(assert (forall ((G Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (and (= G 0) (h15 A B C D E F G H I J K L)) (h19 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h19 A B C D E F G H I J K L) (h5 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h17 A B C D E F G H I J K L) (h5 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h14 A B C D E F G H I J K L) (h20 A B C D E F G H I J K L))))
(assert (forall ((J Int)
         (K Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (L Int))
  (=> (and (<= (- J K) (- 1)) (h20 A B C D E F G H I J K L))
      (h21 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h21 A B C D E F G H I J K L) false)))
(assert (forall ((J Int)
         (K Int)
         (A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (L Int))
  (=> (and (>= (- J K) 0) (h20 A B C D E F G H I J K L))
      (h22 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h22 A B C D E F G H I J K L) (h23 A B C D E F G H I J K L))))
(assert (forall ((A Int)
         (B Int)
         (C Int)
         (D Int)
         (E Int)
         (F Int)
         (G Int)
         (H Int)
         (I Int)
         (J Int)
         (K Int)
         (L Int))
  (=> (h23 A B C D E F G H I J K L) (h24 A B C D E F G H I J K L))))
(check-sat)
